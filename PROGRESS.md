...
**Deploy:** pushed to `main`; no manual `vercel --prod` per the standing "push to GitHub only" direction. **Important:** even after pushing, the live site's Postgres will need the migration applied (`npx prisma migrate deploy` against the real `DATABASE_URL`) before any of this reputation logic can actually run — until then, every route that now calls `updateReputation`/`checkMilestones` will throw against the missing `ReputationLog` table.
