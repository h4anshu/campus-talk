...
**Part 1 — Sentry.** `sentry.client.config.ts`/`sentry.server.config.ts`/`sentry.edge.config.ts` created exactly per spec (`dsn: process.env.SENTRY_DSN`, `tracesSampleRate: 0.1`, `enabled: NODE_ENV === 'production'`), `next.config.mjs` wrapped with `withSentryConfig` (kept ESM — the project already used `.mjs`, task said not to convert format), `app/global-error.tsx` created with `Sentry.captureException` in a `useEffect`, and `lib/api-helpers.ts`'s `handleApiError` now reports genuinely unexpected errors to Sentry (after the `ApiError`/`ZodError` early-returns, so auth failures and validation errors never hit Sentry — only real bugs do), centrally covering all ~41 routes per the task's explicit "don't add Sentry to every route manually" instruction. **Necessary addition beyond the literal spec:** `instrumentation.ts` with a `register()` hook that imports the server/edge config files — confirmed via the build's own deprecation warning that this SDK version (`@sentry/nextjs@10.65.0`) silently never initializes Sentry on the server/edge runtime without it; the 3 config files' contents were kept exactly as specified, this just bridges them in. Client-side config still shows a soft "rename to instrumentation-client.ts" deprecation warning at build time — non-blocking (only affects Turbopack, this project uses webpack) and left as-is per "don't restructure beyond what's asked."

**Part 2 — Upstash rate limiting.** `lib/ratelimit.ts` created exactly per spec: `strictLimiter` (5/min, `ct:strict`), `standardLimiter` (30/min, `ct:standard`), `looseLimiter` (60/min, `ct:loose`), `getClientIp()`, `applyRateLimit()` returning a 429 `Response` or `null`. Applied to all 13 named routes/methods across the 3 tiers, as the first statement inside each route's existing `try` block (survives a Redis network hiccup without altering control flow) — no other route logic touched. `app/api/posts/route.ts` needed two tiers on two methods in the same file (GET=loose, POST=standard); `app/api/notifications/route.ts`'s `GET` had zero parameters previously and needed a `req: NextRequest` added solely to read the client IP. Excluded NextAuth's own route and `admin/login` (bcrypt-protected; the task itself flagged Redis-based limiting there as a lockout risk) — resolved a contradiction in the task's own exclusion rules (it also said skip "any GET already behind `getSessionOrThrow()`", which would have voided half the named loose-tier list) by treating the explicit named-route list as authoritative.

**Part 3 — Vitest.** `vitest.config.ts`, `tests/setup.ts`, and 5 test files created. `tests/setup.ts` mocks Prisma (named `{ prisma }` export, not default — the task's own template mocked a default export that doesn't exist in this codebase), `@/auth`, `@/lib/admin-auth`, `@/lib/createNotification`, `@/lib/ratelimit` (via `importOriginal` passthrough so `getClientIp` stays genuinely real for its own unit test while the limiters are mocked to always allow), and `@sentry/nextjs`. Fixed 3 concrete bugs in the task's own literal test specs rather than copying them as-is: `vote.test.ts`'s request body used a schema shape (`{value:1}`) that doesn't match the real `voteSchema` (`{type:'up'}`); both `vote.test.ts` and `report.test.ts` needed a targeted cast around `vi.mocked(auth).mockResolvedValue(null)` because NextAuth v5's overloaded `auth()` signature makes TS reject a bare `null` mock; `tickets.test.ts`'s original version built a `Request` and only asserted on its `.url` string without ever calling `POST()` — proved nothing — rewritten into 3 real tests (admin reply via `?as=admin`, same-cookie reply without the query param staying `senderRole: USER`, non-admin/non-owner → 403).

**Live verification gap, disclosed rather than faked:** the task's own verification step called for temporarily lowering `strictLimiter` to 2/min and firing 3 real requests at `/api/posts/[id]/report` to confirm a live 429. Attempted this with a disposable college/user/post/session fixture (created and torn down via the app's own `lib/prisma.ts` client) — but `SENTRY_DSN`/`UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN` are **not actually present** in this machine's `.env`/`.env.local`, despite the task text asserting they were already set locally (confirmed by grep across `.env`, `.env.local`, `.env.example`; dev server log showed `[Upstash Redis] Redis client was initialized without url or token`, so `applyRateLimit` threw and every request 500'd before ever reaching the limiter logic). Flagged this to the user rather than fabricating credentials or silently skipping the check; user confirmed the real values exist only in Vercel, so this verification is deferred to post-deploy (check Vercel logs for real 429s under load, and the Upstash dashboard for `ct:`-prefixed keys, and the Sentry dashboard for a deliberately-triggered test error). `strictLimiter` was reverted back to 5/min before committing (the temporary 2/min value never left this machine).

**Verified:** `npx tsc --noEmit` — zero errors. `pnpm test` — 5 files, 13 tests, all green (mocked Redis/Prisma/Sentry). `pnpm build` — succeeds, all ~41 API routes compile as dynamic functions, middleware included. `pnpm-workspace.yaml`'s `allowBuilds` needed `'@sentry/cli': true` (pnpm's build-script approval gate, not the deprecated `package.json#pnpm.onlyBuiltDependencies` field) or `pnpm test`/`pnpm build` failed outright on a fresh install.

---

## Fix — 12 minor findings (m1–m12), all live-verified with disposable fixtures

**Status:** All 12 implemented, typechecked, built, and live-verified against a single shared fixture set (2 colleges, 9 disposable users, several posts/reports) created and torn down this session — deletion counts checked against expected counts before declaring done.

**m1** — `vote`/`save`/`report` routes now catch Prisma P2002 instead of letting it fall through to a raw 500. Deliberately NOT uniform 409-everywhere: `vote` and `report` return 409 (a losing concurrent request's intended outcome — e.g. voting "down" when a concurrent "up" won — genuinely didn't happen, so 409 is honest), but `save` returns the idempotent success `{ isSaved: true }` instead, since the toggle's desired end-state (post is saved) already holds regardless of which concurrent request "won" — 409 there would misrepresent a race as a user-facing problem. Verified live: fired genuine concurrent duplicate requests (`Promise.all`) at each of the 3 routes — vote: one 200 + one 409 with the client's expected message; save: two 200s, idempotent; report: one 200 + one 409 with the exact "You have already reported this post" text the client toast already matches on.

**m2** — `GET /api/posts/[id]` now 404s for non-author/non-admin viewers unless the post is `APPROVED` and college-scoped (matching the main feed's own scoping) or platform-wide (`collegeId: null`); `PATCH` now blocks edits (403) when `status === 'REMOVED'` or `locked === true`, even for the author. Verified: a College B student GET'ing a College A student's APPROVED post → 404; the same for a PENDING post; the author themselves GET'ing their own PENDING post → 200 (viewerIsAuthor bypass intact); removed a post via the new m9 route, then the author's own PATCH attempt on it → 403.

**m3** — Copied the exact `role === 'ADMIN'` refusal from `users/[id]/ban/route.ts` into the report-action route's `WARN_AUTHOR` and `BAN_AUTHOR` branches only (not `REMOVE_POST`, per the task's explicit scope — removing an admin-authored post without banning the account is a legitimate, different action). Verified: created a real second `role: ADMIN` user (distinct from the system "Admin Office" account) with a reported post — both `WARN_AUTHOR` and `BAN_AUTHOR` on it → 403 with the exact same messages `users/[id]/ban` uses.

**m4** — `createNotification` now takes an optional `actorId` and skips the write entirely when `actorId === userId`. Wired `actorId` into every call site that already had an `actorName`/`actorImage` "actor" concept (post vote, comment vote, both comment-route notifications) for defense-in-depth alongside their existing inline guards, plus the one call site that had no guard at all and was the actual confirmed bug — `posts/[id]/collab/route.ts`'s "Team Full" notification, which unconditionally notified the OP about their own action. Verified: OP marked their own collab post complete — zero `COLLAB_SLOT_FILLED` notifications created (would have been 1 before this fix).

**m5** — Search now filters empty tokens out *before* appending the `:*` tsquery suffix, so an all-punctuation query (`"()"`, `"!!!"`) produces a genuinely empty `formattedQuery` string, which the existing `if (formattedQuery)` guard already correctly skips — no new early-return needed once the token list itself is correctly empty. Verified: both queries → 200, `{posts:[],people:[],topics:[]}`; a normal query still returns real matches (no regression).

**m6** — Unread endpoint now validates the parsed date with `isNaN(parsed.getTime())` before using it, falling back to 24h-ago on garbage input. Verified: `announcements=not-a-real-date` → 200 with all 6 space counts computed correctly (no crash), the malformed param's own space fell back to the 24h default while every other space used its own real visit timestamp normally.

**m7** — Reject route gated on `existing.status === 'PENDING'`, mirroring the approve route's own gate. Verified both directions: rejecting an already-APPROVED post → 400, and the post's status confirmed unchanged (`APPROVED`) via direct DB read afterward; rejecting a genuinely PENDING post → still succeeds normally (`REJECTED`).

**m8** — Added `linkUrl`/`refId` to the `REMOVE_POST` author notification. Worth noting: none of the *other* notifications in that same file actually had this either (the task's "unlike every sibling" framing didn't hold on a fresh read) — matched the broader codebase convention instead (`approve`/`reject` routes' `linkUrl: /post/${id}, refId: id` pattern) and left every other notification in the file untouched, since only this one was named in scope. Verified via direct DB read after a real REMOVE.

**m9 — the biggest of the 12.** Built `app/api/admin/posts/[id]/moderate/route.ts` (PATCH, `requireAdmin()`, actions `PIN`/`UNPIN`/`LOCK`/`UNLOCK`/`REMOVE`) plus `hooks/useModeratePost.ts`. **A real architectural mismatch in the task's own premise, found and adapted around:** the task assumed admin already views individual posts somewhere with moderation context (referencing "the C1 ticket-bug pattern"), but `middleware.ts`'s protected-routes matcher includes `/post/:path*`, requiring a NextAuth session cookie — the cookie-only admin panel has no NextAuth session at all, so admin literally cannot reach `/post/[id]` to begin with. There is no existing "admin browses an arbitrary post" surface anywhere in this app; the only two places admin sees real post content are `approvals` (PENDING only) and `reports` (reported only). Given the task's own "no new page, keep minimal" constraint, added the Pin/Lock icon-button controls to the existing `reports` page's `ReportCard` (extended `GET /api/admin/reports`'s response with `postPinned`/`postLocked` so the buttons reflect real current state) rather than inventing a new page — a real, disclosed limitation is that this only covers posts that have been reported at least once; pinning/locking a never-reported post has no UI entry point yet. Did **not** add a duplicate bare "Remove" button on the reports page since a full, reputation-aware `REMOVE_POST` already exists there via the report-action flow. Verified live: all 5 actions (PIN/UNPIN/LOCK/UNLOCK/REMOVE) via direct PATCH calls, each persisted correctly (confirmed via the route's own response and cross-checked against the m2 test above); the REMOVE path's author notification confirmed present with correct `linkUrl`/`refId`, reusing the exact m8 pattern.

**m10** — Confession-space comments now auto-anonymize. Client: the top-level composer in `app/(main)/post/[id]/page.tsx` sends `anonymous: post.space === 'confession'`. Server: `app/api/comments/route.ts` only *honors* that flag when `post.space === 'CONFESSION'` — a spoofed `anonymous: true` on a direct API call to a normal post is silently ignored server-side, which is a real, deliberate hardening beyond the task's literal "accept and persist" instruction (blind trust would let anyone post anonymously anywhere in the app). Also exported `ANONYMOUS_AUTHOR` from `lib/serializers.ts` so this route's manually-built response object masks the author the same way `serializeCommentNode` already does on every subsequent GET — otherwise the comment's *own creation response* would have shown the real name for one render before a refetch fixed it. **Disclosed scope boundary:** only the top-level composer is wired; nested replies (`CommentItem.tsx`'s reply composer) are not, since doing so would require threading a new `postSpace` prop through `CommentThread`/`CommentItem`/`AnswerCard` — a materially bigger change than the task's own verification checklist (which only tests a top-level comment) called for. Verified: a real comment on the confession post → `author: "Anonymous"` in both the creation response and a subsequent GET; a spoofed `anonymous:true` on a normal APPROVED post → real author name shown, confirming the server-side gate actually holds.

**m11** — Group status is now `PENDING` if *any* report in the group is still `PENDING`, regardless of recency. Verified with the actually-discriminating case (the one case that matters): a post with an **older PENDING** report and a **newer DISMISSED** report — the old "latest wins" logic would have shown this group as `DISMISSED`; confirmed it now correctly shows `PENDING`. (Also checked the non-discriminating case — newer-PENDING/older-DISMISSED — which both old and new logic agree on and therefore proves nothing on its own; the inverse case above is the one that actually matters.)

**m12** — Landing stats now return `degraded: false` on success and `degraded: true` on the DB-failure fallback, alongside the existing (unmodified) error log — an honest, inspectable signal that a given response used the static baseline rather than a live count, without redesigning `Footer.tsx`'s already-vague "12K+"-style rounded display (which never claimed real-time precision in the first place). Verified the success path returns `degraded: false`. **Not verified — deliberately not attempted:** the actual DB-failure path, since forcing a real outage against the shared Neon database risked disrupting other legitimate use of that same database; this one was confirmed by direct code review instead (the catch block is a simple, low-risk static return, unlike the concurrency-sensitive fixes elsewhere in this batch where live proof mattered much more).

**Scope discipline:** every file touched maps directly to one of the 12 items (21 files total, including the 2 new ones for m9) — confirmed via `git status --porcelain` before and after.

**Verified:** `npx tsc --noEmit` — zero errors. `npx next build` — succeeds (the "Dynamic server usage... cookies/headers" messages during build are expected Next.js notices for every authenticated route in this app, not errors). Cleanup: all 9 disposable users deleted, count matched exactly against the list printed before deletion; final row counts (`posts:1, users:3, votes:0, reports:1, comments:0, reputationLogs:4`) match the exact pre-existing baseline confirmed in prior rounds.

---

## Fix — M10 now fully closed (partial unique index) + M8 verified live on production

**Status:** M10's race is now genuinely closed — proven with the same `Promise.all` real-concurrency test that exposed it last round, this time surviving it. M8 was verified with 5 real requests against the actual deployed Vercel production app: 5/5 succeeded, 0 inconsistency.

**M10 — partial unique index, applied only for verification then dropped; the real migration is still pending your `migrate deploy`.** `prisma/migrations/20260712120000_add_milestone_unique_index/migration.sql` adds `CREATE UNIQUE INDEX "unique_milestone_per_user" ON "ReputationLog" ("userId", "reason") WHERE "reason" IN ('FIRST_POST', 'TEN_HELPFUL_ANSWERS', 'FIFTY_LIKES_TOTAL')` — all 3 reasons from `lib/reputation.ts`'s own "Milestones (one-time)" section (not just the 2 `checkMilestones.ts` currently checks), grepped directly rather than guessed. Documented on the `ReputationLog` model in `schema.prisma` since Prisma's schema DSL has no way to express a partial/filtered index — a future `migrate dev`/`db pull` won't see it and must not be allowed to "fix" it as drift.

**A real bug in the task's own Step 3 snippet, caught and fixed before implementing it:** the snippet wraps the per-milestone try/catch inside one shared `$transaction` looping over all candidates. That's unsafe — once `reputationLog.create` throws a genuine Postgres unique-violation inside a transaction, Postgres marks that *whole* transaction aborted; every later statement in the *same* transaction fails too, even ones wrapped in their own try/catch (there's no implicit savepoint per iteration). A user crossing two milestones in the same event would have the first one's lost race take down the second one's chance to be checked/awarded in that same call. Fixed by giving each candidate milestone its own independent `prisma.$transaction(...)` call instead of one shared transaction with a `for` loop inside it.

**Also deliberately bypasses the shared `updateReputation` helper for this path** — that helper fires the log-create and the reputation-increment via `Promise.all`, so a losing `create` could still leave the `increment` applied (partial state: reputation bumped with no log entry to explain it). The milestone path now does `create` first, and only calls `user.update` if `create` fully succeeds.

**Verified — the actual race, not just the code:** could not run the real migration myself (task explicitly says defer `migrate deploy` to the user, matching this session's established norm for every hand-written migration). Instead created a **temporary** unique index — identical DDL, different name (`temp_verify_milestone_unique`) — via a throwaway script, entirely outside Prisma's migration tooling, to prove the fix against a genuine constraint without touching migration history:
- Pre-check: zero pre-existing duplicate `(userId, reason)` milestone rows in the live DB (safe to add the index at all).
- Built a user with exactly 50 qualifying upvotes, fired `checkMilestones(userId)` twice via genuine `Promise.all` — **exactly one `FIFTY_LIKES_TOTAL` entry**, reputation exactly `25` (last round: two entries, reputation `50`).
- Regression check: cast 2 separate real upvotes (via the live vote API) on the same author's 2 different posts — **2 `POST_LIKED` entries coexist with no constraint violation** — confirming the partial index doesn't touch repeatable events.
- Dropped the temporary index immediately after. `prisma migrate status` confirms the real named migration is still listed as **not yet applied**.
- **Honest answer to "is the race fully closed now": yes**, as far as this test can prove — a real Postgres unique constraint, not merely application-level timing, now makes the second concurrent award structurally impossible to persist. The remaining caveat is the same one that applies to any DB constraint: it protects the row-level guarantee, not anything above the database (e.g., if a future refactor changed `updateReputation` semantics elsewhere to not go through this exact create-then-update-if-create-succeeds path). Within the scope of what changed here, this is a real, structural fix, not merely a narrowed window.

**M8 — verified live on the actual deployed production app**, per the task's explicit ask (local dev can't reproduce serverless-freeze behavior; it's a long-lived process, not a Lambda). Confirmed via `vercel ls`/`vercel inspect` (authenticated as `h4anshu`, `campus-talk` project) that the most recent Production deployment was `● Ready` and aliased to `campus-talk-gamma.vercel.app`. Rather than lowering a milestone threshold in code (which the task offered as an option but which would have required two extra production deploys — the temporary change and its revert), pre-seeded 49 qualifying upvotes per test user via direct DB writes (same shared Neon database production uses), then triggered the real, final 50th vote through the **actual live HTTP API** — the genuine code path being tested, since only that request runs inside a real Vercel serverless invocation. One environment gotcha hit and resolved: the session cookie name differs in production — `authjs.session-token` (local, plain HTTP) 401'd; `__Secure-authjs.session-token` (the `__Secure-` prefix Auth.js applies over HTTPS) worked.

**Result: 5 attempts, 5 successes, 0 inconsistency.** Each of 5 disposable test users, after their live-triggered vote, showed `voteCount: 50, reputation: 30 (5 POST_LIKED + 25 FIFTY_LIKES_TOTAL), milestoneLogCount: 1` — exactly correct, every single time. No milestone notification exists to check for this specific event (`checkMilestones` never calls `createNotificationSafe` — confirmed by reading the function; it only logs + increments reputation), so that part of the task's ask doesn't apply here. Being precise about what 5/5 does and doesn't prove: it's strong empirical evidence, not a mathematical guarantee that this could never fail under different load/timing — but the actual mechanism providing confidence is structural, not just empirical: since the call is now `await`ed, the HTTP response literally cannot be sent until the promise settles, so Vercel's post-response freeze can no longer race it. The 5/5 result confirms this holds in practice on the real runtime, not just in theory.

**Scope discipline:** Part 1 touched exactly `lib/checkMilestones.ts`, `prisma/schema.prisma` (comment only), and the new migration file. Part 2 required no code changes — verification only.

**Verified:** `npx tsc --noEmit` — zero errors. `npx next build` — succeeds. Every test fixture (Part 1: 5 users across two tests; Part 2: 6 users across 5 iterations) was created with a distinctive, trackable email prefix (`verify4-`, `verify5-`), listed before deletion, and deletion counts checked against expected counts. Final sanity check after both parts: `posts:1, users:3, votes:0, reputationLogs:4` — the exact pre-existing baseline, confirmed by name to be real data from 2026-07-08, untouched.

**Not applied — by design, per the task's explicit instruction:** `npx prisma migrate deploy` for `20260712120000_add_milestone_unique_index`. Run it when ready; it's a lossless `CREATE UNIQUE INDEX` (no column changes, no data migration) and was pre-verified (via the temporary index above) to be safe against the current data with zero conflicts.

---

## Fix — 3 concurrency bugs (M7, M8, M10) — M10 reduces but does NOT fully close its race, disclosed below

**Status:** All 3 implemented, typechecked, built. M7 and M10 were verified with genuine concurrent requests (`Promise.all`, not sequential curl calls) against disposable, tracked fixtures — this is the first round where the actual race condition itself, not just the surrounding logic, was exercised live.

**M7 — collab slot count is now a server-applied atomic delta, not a client-computed absolute value.** `app/api/posts/[id]/collab/route.ts`'s `PATCH` now accepts `{ action: 'increment' | 'decrement' }` and applies it via `prisma.post.updateMany({ where: { id, collabFilledSlots: { lt/gt: ... } }, data: { collabFilledSlots: { increment/decrement: 1 } } })` — the `where` guard means the call either fully applies or matches zero rows (409), with no read-modify-write window in between. `isClosed`/`deadline` stay a separate, untouched direct-set path (the UI never sends them alongside `action`). Propagated through `hooks/useCollabUpdate.ts` (now typed `{ action } | { isClosed?, deadline? }`) and `components/post/CollabSlotBar.tsx`'s two slot buttons (now send `{ action: 'increment' }`/`{ action: 'decrement' }` instead of computing `filledSlots ± 1` client-side). **One disclosed behavior change:** the old code auto-closed the post (`collabIsClosed = true`) as a side effect of an increment reaching `collabTotalSlots`; the task's specified fix doesn't reproduce this, and combining it into a single atomic statement would have reintroduced exactly the kind of compound-update risk this fix is meant to eliminate, so it was dropped — the OP can still close the post explicitly via the always-available "Mark as complete" button. **Verified with real concurrency:** fired two `PATCH .../collab` `{action:'increment'}` requests via `Promise.all` against a 2/4-slot test post — both succeeded, and a direct DB read afterward confirmed `collabFilledSlots: 4` (both increments landed; the old code would have lost one, landing at 3). Also confirmed incrementing at 4/4 → 409 "All slots already filled", and decrementing at 0 → 409 "Already at zero".

**M8 — 5 fire-and-forget writes now awaited (with try/catch so a failure there never fails the primary request):** `checkMilestones(...).catch(...)` in `app/api/posts/[id]/vote/route.ts`, `app/api/comments/[id]/vote/route.ts`, and `app/api/comments/[id]/accept/route.ts`; the un-awaited `createNotificationSafe(...)` in `app/api/posts/[id]/collab/route.ts` (now naturally `await`ed as part of the M7 rewrite of that same file — matches the 3 other `createNotificationSafe` call sites in the codebase, which were already correctly awaited, so no change needed there); and the `lastActiveAt` update inside `getSessionOrThrow` in `lib/api-helpers.ts`, which runs on effectively every authenticated request in the app. **Not verified — genuinely can't be from here:** the actual failure mode (Vercel freezing the function instance right after the response is sent) only reproduces on the deployed serverless runtime, not local dev, which is a long-lived process where every promise always gets to run regardless of whether it's awaited. The task's own verification step explicitly asks for a check "on the DEPLOYED production app" — that's the user's to run.

**M10 — the idempotency check now runs inside the same transaction as each milestone award, immediately before its `ReputationLog.create`, per-reason rather than one bulk check for every candidate milestone up front.** This is a real, verified narrowing of the race window — but **explicitly does not fully close it**, exactly as the task anticipated and asked to be reported honestly rather than overstated: `reason` is shared with repeatable, non-milestone events (`POST_LIKED`, `COMMENT_LIKED`, etc.), so the blanket `@@unique([userId, reason])` constraint the task offered as the "preferred" fix would break those and was correctly not applied; the task's own fallback (the transaction-scoped check implemented here) still races under Postgres's default `READ COMMITTED` isolation, since two concurrent transactions' `SELECT`s can both complete before either commits its `INSERT` — nothing at the database level stops that. **Verified this is a real, present gap, not a theoretical one:** built a disposable user with exactly 50 real qualifying upvotes (the `FIFTY_LIKES_TOTAL` threshold) across 50 disposable test posts, then called `checkMilestones(userId)` twice via genuine `Promise.all` (no `HTTP` layer, direct function calls, maximum concurrency) — **both calls awarded the milestone**, producing two `+25 FIFTY_LIKES_TOTAL` `ReputationLog` entries and a reputation of `50` instead of the intended one-time `25`. This is the same class of race that existed before (just with a narrower window than the old one-check-for-everything version), not a new one introduced by this change. A full fix needs either `SERIALIZABLE` isolation for this specific transaction or a unique index scoped to just the milestone-type reasons (e.g. a Postgres partial unique index) — neither was implemented here, per the task's own explicit scope boundary favoring the safer, narrower application-level mitigation over a schema change that needed more care than "implement the fix as specified" authorized.

**Scope discipline:** touched exactly the files the 3 fixes required (including the two client-side files M7's API-contract change necessitated) — confirmed via `git status --porcelain` before and after.

**Verified:** `npx tsc --noEmit` — zero errors. `npx next build` — succeeds, all routes compile. Every test fixture (M7: 1 OP + 1 collab post; M10: 1 milestone candidate + 1 voter + 50 posts + 50 votes) had its ID recorded before creation, and cleanup's `deleteMany` count was checked against the expected count before declaring done. A final row-count check confirmed the app returned to exactly its pre-existing baseline (1 real post, 3 real users, 0 votes).

---

## Fix — 5 more audit findings (M3, M4, M9, M11, M12), all live-verified with disposable fixtures

**Status:** All 5 implemented exactly as specified, typechecked, built, and live-verified. Same fixture discipline as the last round: every test user/post/comment/report/session created this session had its ID recorded before any deletion, and cleanup's `deleteMany` count was checked against the expected count before declaring done. A final row-count check confirmed the one leftover `Post`/`Report` row (real data from 2026-07-08, days before this session) was untouched.

**M3:** `app/api/comments/[id]/accept/route.ts` now short-circuits with `{ success: true, alreadyAccepted: true }` when `comment.accepted` is already true, placed after the existing ownership/type guards but before the unaccept-all/re-accept/award transaction. Verified: accepted the same answer twice — second call returned `alreadyAccepted: true`, and `ReputationLog` showed exactly one `ANSWER_ACCEPTED` entry (+15), not two.

**M4:** `hooks/useNotifications.ts`'s three raw `fetch()` calls (`useNotifications`, `useMarkAllRead`, `useMarkOneRead`) replaced with the shared `fetchJson` from `lib/api-client.ts` — the fourth `fetch()` in `useNotificationRealtime`'s Supabase-config setup was left untouched since it wasn't part of the cited root cause or fix. `useMarkOneRead` now captures the pre-mutation cache in `onMutate` and rolls it back in a new `onError`. Verified: hit `/api/notifications` unauthenticated → real `401 {"error":"Unauthorized"}`; traced `components/layout/Navbar.tsx:94`'s `notifications.filter(...)` (which runs unconditionally, no `isError` gate) — before this fix a 401 would have resolved as the "successful" data (an object, not an array) and crashed that `.filter` call; now `fetchJson` throws on non-`ok` responses, so the query errors, `data` stays `undefined`, and the Navbar's own `= []` destructuring default keeps it safe.

**M9:** `app/api/admin/users/[id]/route.ts` now finds all of the to-be-deleted user's outgoing `Vote` rows (with post/comment author) *inside* the same transaction as the delete, and reverses each one's reputation grant via the existing `updateReputation` helper — `POST_LIKED_REMOVED`/`POST_DISLIKED_REMOVED` for post votes, `COMMENT_LIKED_REMOVED` for comment upvotes (comment downvotes never granted reputation in the first place, matching the comment-vote route, so nothing to reverse there) — rather than a raw, unlogged `reputation: increment` as the task's illustrative snippet suggested. Using the established helper keeps this fully part of the normal `ReputationLog` audit trail instead of introducing a second, untracked way to change reputation. Verified end-to-end: created a voter + a post author + a comment author, cast one upvote each (post author → +5, comment author → +3, confirmed via direct query), deleted the voter through the real admin DELETE route, and confirmed both authors' reputation returned to exactly `0` with a full, symmetric `ReputationLog` (`+5`→`-5`, `+3`→`-3`).

**M11:** Added the same reporter-notification loop the other 4 report actions already have to the `DISMISS` branch (previously the only action that silently docked reputation with no bell notification) — same `REPORT_ACTION_TAKEN` type and body text `MARK_REVIEWED` already uses, since both actions mean "reviewed, no violation found." Omitted the task snippet's `linkUrl: null` — `createNotificationSafe`'s `linkUrl` field is `string | undefined`, not nullable, and every sibling call in this same file omits it entirely rather than passing null. Verified: dismissed a real test report — the reporter received a `REPORT_ACTION_TAKEN` bell notification with the exact expected text, alongside the pre-existing `-3 REPORT_FALSE` log entry that gives it context.

**M12:** `auth.ts` gained a `buildAnchoredDomainRegex` helper that wraps any `College.domainPattern` value in `^...$` unless it's already anchored, used in place of the raw `new RegExp(pattern, 'i')` construction in `findMatchingCollege`. This is a read-only-verification fix per the task's own instructions (no live signup test needed) — verified via a standalone Node script covering exactly the cases the task specified: an unanchored pattern now correctly rejects a lookalike domain (`bbdu.ac.in.evil-domain.com`) that it would have matched before; already-anchored real seed patterns are unaffected; and — the regression check that actually matters here — the real `BBD Group` wildcard pattern (`^bbd[a-z0-9]*\.ac\.in$`) still correctly matches a genuine subsidiary domain like `bbdnitm.ac.in`, so real student signup isn't broken by this change.

**Scope discipline:** touched exactly the 5 files named in the task — confirmed via `git status --porcelain` before and after.

**Verified:** `npx tsc --noEmit` — zero errors. `npx next build` — succeeds, all routes compile. All 5 verification checklists passed against live (not merely forged-single-session) test scenarios.

---

## Fix — 5 audit findings (M13, M6, M5, M2, M1), all live-verified with disposable fixtures

**Status:** All 5 fixes implemented exactly as specified, typechecked, built, and live-verified — this time using only test data created and tracked by this session (learned from the prior turn's data-loss incident: every fixture below was created fresh, its ID recorded up front, and cleanup deleted only those exact IDs — never "the most recent row").

**M13 (highest priority — cross-college privacy leak):** `app/api/search/route.ts`'s user-search branch had no `collegeId` filter while the post-search branch a few lines above it did. Added `collegeId: session.user.collegeId` to the user query's `where`. Verified: created a user in College A (searcher) and a distinctively-named user in College B (target) with a real NextAuth session for the searcher — searching the target's name returned `people: []`; searching the searcher's own name still found them (no regression).

**M6:** `app/api/posts/route.ts` now rejects `space: 'announcements'`/`'events'` with 403 unconditionally on the student-facing route (confirmed `app/api/admin/posts/route.ts` is genuinely the separate admin-posting path, so no legitimate case needed preserving here). Verified: POST with `space: 'announcements'` → 403 with the exact message; a normal DISCUSSION post from the same session still → 201 (no regression).

**M5:** `app/api/comments/route.ts` now fetches `locked`/`collabIsClosed`/`status` alongside the existing post lookup and rejects (403) before comment creation when any of those block it — closing the gap where only the UI (`CommentComposer.tsx`) disabled the input. Verified: a comment POST against a `locked: true` test post → 403 "This post is locked..."; the same session commenting on a normal unlocked APPROVED post → 201 (no regression).

**M2:** All 5 branches of `app/api/admin/reports/[postId]/action/route.ts` (`REMOVE_POST`, `WARN_AUTHOR`, `BAN_AUTHOR`, `MARK_REVIEWED`, `DISMISS`) now gate their `report.updateMany` on `status: 'PENDING'` and check the returned `count` — reputation awards, the post/user state mutation, and notifications all now only fire when `count > 0` (i.e., this call actually transitioned something out of PENDING); `count === 0` returns `{ success: true, alreadyResolved: true }` immediately. Postgres's row-level UPDATE locking makes this correct under real concurrent double-clicks, not just sequential retries. Verified end-to-end: created a post with 2 reporters, called `REMOVE_POST` (200, no `alreadyResolved`), called `REMOVE_POST` again (`alreadyResolved: true`), called `DISMISS` on the same now-resolved report (`alreadyResolved: true`) — then queried `ReputationLog` directly and confirmed **exactly one** entry per user (the author's -10, each reporter's +8) despite 3 total calls.

**M1:** `app/api/spaces/unread/route.ts`'s per-space count query filtered `collegeId` alone; admin-authored posts (Announcements/Events) are created with `collegeId: null` (platform-wide) and were invisible to this query even though the main feed's own read path already handles `null` via `OR`. Changed to the same `OR: [{ collegeId }, { collegeId: null }]` pattern. Verified: created an admin-authored (`collegeId: null`) Announcements post, queried unread as a real College A student with an old last-visited timestamp — `announcements: 1` came back correctly (would have been `0` before the fix).

**Scope discipline:** touched exactly the 5 files named in the task and nothing else — no refactors, no drive-by cleanup, confirmed via `git status --porcelain` before and after.

**Verified:** `npx tsc --noEmit` — zero errors. `npx next build` — succeeds, all routes compile. All 5 live-verification checklists from the task passed against real (not forged-only) request scenarios: a genuine NextAuth database session for search/posts/comments, a forged-but-legitimately-signed admin cookie (same technique as prior sessions, since the real admin password isn't available here) for the report-action test.

**Cleanup discipline (post-incident):** every fixture created this session — 6 users, 2 posts (locked + normal), 1 reported post, 2 reports, 1 announcement post, 1 comment, 1 session — had its ID recorded in the setup script's own output before any deletion, and cleanup's `deleteMany` result counts were checked against the exact expected counts (6 users deleted = 6 expected, 1 announcement post deleted = 1 expected) before declaring cleanup complete. Confirmed via a final row-count sanity check that no pre-existing data was touched.
