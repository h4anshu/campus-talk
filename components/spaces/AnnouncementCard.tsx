...
export default function AnnouncementCard({ post }: AnnouncementCardProps) {
  ...
  return (
    <motion.div
      ...
      className={`cursor-pointer overflow-hidden rounded-card border-[0.5px] bg-[var(--bg-surface)] transition-colors hover:border-[var(--border-med)] ${
        post.pinned ? 'border-[var(--accent-border)]' : 'border-[var(--border)]'
      }`}
    >
      {thumbnail && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={thumbnail} alt="" className="h-[140px] w-full object-cover" />
      )}
